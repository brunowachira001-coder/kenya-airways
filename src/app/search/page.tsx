...
function SearchContent() {
  ...
  const [expanded, setExpanded] = useState<{ id: string; cls: "economy" | "business" } | null>(null);

  const toggle = (id: string, cls: "economy" | "business") =>
  ...
  const handleSelect = (flight: Flight, tier: "economy" | "business", tierIndex: number) => {
    ...
    // Clear return flight on outbound selection
      ...
      <div className="bg-white border-b border-gray-200 sticky top-0 z-30 shadow-sm">
        <div className="max-w-[1100px] mx-auto px-4 py-3 flex flex-wrap items-center gap-x-4 gap-y-2">
          ...
          <div className="flex items-center gap-3 ml-auto">
            <Link
              href="/booking/fare-select"
              className="flex items-center gap-1.5 text-xs font-semibold text-[#0d0d0d] hover:text-[#ed1c24] transition-colors"
            >
              ...
              <div key={flight.id} className="bg-white rounded-xl shadow-sm overflow-hidden border border-[#e8e8e8]">
                ...
                <div className="flex flex-col sm:flex-row">
                  ...
                  <div className="flex border-t sm:border-t-0 sm:border-l border-gray-100 sm:w-[280px]">
                    {/* Economy */}
                    <button
                      onClick={() => toggle(flight.id, "economy")}
                    ...
                    >
                      ...
                      <ChevronDown
                        className={`w-4 h-4 text-gray-400 mt-1 transition-transform duration-200 ${
                          expanded?.id === flight.id && expanded.cls === "economy" ? "rotate-180" : ""
                        }`}
                      />
                    </button>
                    ...
                    {/* Business */}
                    <button
                      onClick={() => toggle(flight.id, "business")}
                    ...
                    >
                      ...
                      <ChevronDown
                        className={`w-4 h-4 text-gray-400 mt-1 transition-transform duration-200 ${
                          expanded?.id === flight.id && expanded.cls === "business" ? "rotate-180" : ""
                        }`}
                      />
                    </button>
                  </div>
        ...
        <div className="max-w-[1000px] mx-auto flex flex-col gap-6">
          <div className="flex flex-wrap justify-center gap-4 sm:gap-8 text-sm text-white font-semibold">
            {["Loyalty Program", "Travel Requirements", "Special Assistance", "Contact Us"].map(l => (
              <span key={l} className="hover:text-gray-300 cursor-pointer transition-colors">
                {l}
              </span>
            ))}
          </div>
